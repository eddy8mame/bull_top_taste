...
export default function Footer({ location }: Props) {
  ...
  return (
    <footer className="bg-[#031109] text-white/50 py-12 px-6 border-t border-white/10">
      <div className="max-w-5xl mx-auto text-center space-y-5">

        {/* Brand name */}
        <p className="text-white font-serif text-lg font-medium">{restaurantName}</p>
        ...
        <div className="flex items-center justify-center gap-3">
          <a href={facebook} target="_blank" rel="noopener noreferrer" aria-label="Facebook"
             className="w-9 h-9 rounded-full bg-white/10 text-white/70 flex items-center justify-center hover:bg-white/20 transition-colors">
            <FacebookIcon />
          </a>

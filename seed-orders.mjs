...
const INSTRUCTIONS = ['Extra spicy', 'Allergy: Peanuts', 'Sauce on the side', 'No utensils', null, null, null, null];
...
function generateModifiers() {
  ...
  // 1. Size Choice (Always present on these mains)
  if (Math.random() > 0.5) {
    modifiers.push({ _key: `mod-${randomId()}`, groupName: "Size Choice", selections: "Large +$3.50" });
    upchargeTotal += 3.50;
  } else {
    modifiers.push({ _key: `mod-${randomId()}`, groupName: "Size Choice", selections: "Small" });
  }

  // 2. Side Choice (Always present)
  const sides = [
  ...
  modifiers.push({ _key: `mod-${randomId()}`, groupName: "Side Choice", selections: getRandom(sides) });

  // 3. Sauce Additions (30% chance)
  if (Math.random() > 0.7) {
    const sauces = ["Jerk Sauce +$0.75", "Fry Chicken Sauce +$0.75", "Jerk Sauce +$0.75, Fry Chicken Sauce +$0.75"];
    const selection = getRandom(sauces);
    modifiers.push({ _key: `mod-${randomId()}`, groupName: "Sauce Additions", selections: selection });
    // Add 0.75 for each sauce selected
    upchargeTotal += (selection.match(/\+\$0\.75/g) || []).length * 0.75;
  }

  // 4. Recommended Sides & Apps (20% chance)
  if (Math.random() > 0.8) {
    const apps = [
      { text: "Plantain (Small) +$6.98", price: 6.98 },
      { text: "Plantain (Large) +$8.98", price: 8.98 }, // 6.98 + 2.00 sub-modifier
      { text: "Cabbage +$4.99", price: 4.99 }
    ];
    const app = getRandom(apps);
    modifiers.push({ _key: `mod-${randomId()}`, groupName: "Recommended Sides & Apps", selections: app.text });
...
async function seedOrders() {
  ...
  for (let i = 1; i <= ORDER_COUNT; i++) {
    ...
    const startedAt = ['kitchen', 'floor', 'completed'].includes(status) ? getRelativeTime(orderAgeMins - 5) : undefined;
    const readyAt = ['floor', 'completed'].includes(status) ? getRelativeTime(orderAgeMins - 20) : undefined;
    ...
    try {
      ...
      console.log(`[${i}/${ORDER_COUNT}] Created ${status.toUpperCase()} order ($${mockOrder.total})`);

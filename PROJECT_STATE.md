...
| Cart panel UI overhaul | Complete except pre-populated modifier modal on cart item tap (subtask 7 — next). All other subtasks shipped. |
...
## System Changelog
* **v1.4.6 (Current):** Added featured items quick-add panel to empty cart state. 2×2 grid sourced from Sanity location document. Items with required modifiers open ModifierModal inline. Direct add for items with no required modifiers.
